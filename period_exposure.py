...
if __name__ == "__main__":
    ...
    for i, band in enumerate(['B', 'V', 'R', 'I']):
        ...
        for sub in ['', 'results/']:
            ...
            unbinned_path = OUTPUT_DIR / f"{sub}unbin_results_{band}.fits"
            if path.exists():
                binned_table = table.Table.read(path)
                unbinned_table = table.Table.read(unbinned_path)
                found = True
                break
        ...
        unbinned_time = Time(unbinned_table['jd'], format='jd')

        # Filter for recent data (CUTOFF) and non-NaNs
        mask = (time > CUTOFF) & (~np.isnan(binned_table['flux']))
        unbinned_mask = (unbinned_time > CUTOFF) & (~np.isnan(unbinned_table['flux']))
        
        t_data = time[mask]
        y_data = binned_table['flux'].data[mask]
        dy_data = binned_table['intraday_std'].data[mask]

        t_unbinned = unbinned_time[unbinned_mask]
        y_unbinned = unbinned_table['flux'].data[unbinned_mask]
        dy_unbinned = unbinned_table['error'].data[unbinned_mask]
        ...
        ls_unbinned = LombScargle(t_unbinned, y_unbinned, dy_unbinned)
        power_unbinned = ls_unbinned.power(freq_grid)

        # Plotting
        axs[i].plot(freq_grid.value, power, c=COLORS[band], lw=LW - 0.5, ls='-.', alpha=0.5, label=f'{band} band')
        axs[i].plot(freq_grid.value, power_unbinned, c=COLORS[band], lw=LW, label=f'{band} band')
    ...
    all_unbinned_table = table.Table.read(OUTPUT_DIR / "unbin_results_all.fits")
    mask = (Time(all_table['jd'], format='jd') > CUTOFF) & (~np.isnan(all_table['flux']))
    unbinned_mask = (Time(all_unbinned_table['jd'], format='jd') > CUTOFF) & (~np.isnan(all_unbinned_table['flux']))
    ...
    t_all_unbinned = Time(all_unbinned_table['jd'][unbinned_mask], format='jd')
    y_all_unbinned = all_unbinned_table['flux'].data[unbinned_mask]
    dy_all_unbinned = all_unbinned_table['error'].data[unbinned_mask]
    b_all_unbinned = np.array([BAND_MAP[b] for b in all_unbinned_table['band'][unbinned_mask]])

    ls_multi = LombScargleMultiband(t_all, y_all, b_all, dy_all)
    power_multi = ls_multi.power(freq_grid)

    ls_multi_unbinned = LombScargleMultiband(t_all_unbinned, y_all_unbinned, b_all_unbinned, dy_all_unbinned)
    power_multi_unbinned = ls_multi_unbinned.power(freq_grid)

    ax_m = axs[4]
    ax_m.plot(freq_grid.value, power_multi, c='k', lw=LW - 0.5, ls='-.', alpha=0.5, label='Joint Multiband')
    ax_m.plot(freq_grid.value, power_multi_unbinned, c='k', lw=LW, label='Joint Multiband')
    ...
    save_path = OUTPUT_DIR / f"periodogram_exposure_{TARGET}_{MIN_PERIOD}-{MAX_PERIOD}_n{N_SAMPLES}.pdf"
    plt.savefig(save_path)  # , dpi=200)
    print(f"Exposure periodogram saved to {save_path}")
    # plt.show()
    plt.close()
