...
The idea of this script is to bin `mag_from_onerel` with `combine_mags`
...
if REQUIRE_ROW_COMPLETE:
    print('Fully complete rows only. Missing any comparison star will disqualify an exposure.')
...
for fn in TARGET_FN:
    ...
    for band in ['B', 'V', 'R', 'I']:
        directmag_table = table.Table.read(
            f'tables/mag_from_onerel/directmag_masked_field={fn}_target={phot_target}_band={band}.fits'
        )
