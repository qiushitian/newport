...
if __name__ == "__main__":
    ...
    coord = newport.TARGET_SKYCOORD[target]
    SCI_PATH = Path(
        f'/Volumes/emlaf/westep-transfer/mountpoint/space-raw/{target.replace("_", " ").replace("-", " ")}'
    )
    CALIB_PATH = Path('/Volumes/emlaf/westep-transfer/mastercalib-2025-no_flat')
    WRITE_PATH = Path(f'tables/list_runs/{target}/phot_gaia_run')
    ...
    # mask_gaia_table = np.array([False, False, False, False,  True, False, False, False,  True,
        ...
        for file in file_list:  # no tqdm
            ...
            try:
                ...
                with fits.open(file.with_suffix('.fts'), output_verify='warn') as hdul:
                    hdu = hdul[0]
                    data_err = newport.error_func(hdu.data)
                    data = CCDData(
                    ...
                    # aper_size = 2
                    aper_size = newport.get_fwhm_nanmin(raw_aperstats[~invalid_aper])
